...
# 📍 Check of script in frontend folder zit
if [ ! -f "index.html" ] || [ ! -f "tailwind.config.js" ] || [ ! -d "src" ]; then
  echo "❌ Fout: Voer dit script uit in de frontend-map!"
  exit 1
fi

echo "✅ Frontend-map bevestigd, start installatie..."

# ✅ Google Fonts Barlow toevoegen
if ! grep -q "fonts.googleapis.com/css2?family=Barlow" index.html; then
  ...
  ' index.html
  echo "✅ Fontlink toegevoegd aan index.html"
fi

# ✅ fontFamily toevoegen aan Tailwind config
...
# ✅ index.css met darkmode en font
...
echo "✅ Alles correct ingesteld. Start nu je app met: npm run dev"
